...
export default function CustomFieldsManager() {
  const { fields, setFields } = useCustomFields();
  ...
  const handleSave = () => {
    ...
    if (trimmedName.length > 100) { toast.error("Field name must be less than 100 characters"); return; }
    ...
    if (editingField) {
      setFields((prev) =>
        prev.map((f) => f.id === editingField.id ? { ...f, name: trimmedName, type: fieldType, required, options } : f)
      );
                    ...
                    <TableRow key={field.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className="gap-1 text-xs font-normal">
                          {field.type === "text" ? <><Type className="h-3 w-3" /> Free Text</> : <><ChevronDown className="h-3 w-3" /> Dropdown</>}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {field.required ? <Badge variant="default" className="text-xs font-normal">Yes</Badge> : <span className="text-xs text-muted-foreground">No</span>}
                      </TableCell>
                      <TableCell>
                        {field.options ? (
                          <div className="flex flex-wrap gap-1">
                            {field.options.slice(0, 3).map((opt) => <Badge key={opt} variant="outline" className="text-xs font-normal">{opt}</Badge>)}
                            {field.options.length > 3 && <span className="text-xs text-muted-foreground">+{field.options.length - 3}</span>}
                          </div>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => handleDelete(field.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { if (!open) resetForm(); setDialogOpen(open); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Define a new field that admins can add to records across the platform.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Field Name</Label>
              <Input placeholder="Permit Number" value={fieldName} onChange={(e) => setFieldName(e.target.value)} maxLength={100} />
            ...
            <div className="space-y-1.5">
              ...
              <Select value={fieldType} onValueChange={(v) => setFieldType(v as "text" | "dropdown")}>
                ...
                <SelectContent>
                  <SelectItem value="text"><span className="flex items-center gap-2"><Type className="h-3.5 w-3.5" /> Free Text</span></SelectItem>
                  <SelectItem value="dropdown"><span className="flex items-center gap-2"><ChevronDown className="h-3.5 w-3.5" /> Dropdown (Fixed Values)</span></SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{fieldType === "text" ? "Users can enter any value." : "Users select from predefined options."}</p>
            </div>
            {fieldType === "dropdown" && (
              <div className="space-y-1.5">
                <Label className="text-xs font-medium">Dropdown Options</Label>
                <Textarea placeholder={"Option 1\nOption 2\nOption 3"} value={dropdownOptions} onChange={(e) => setDropdownOptions(e.target.value)} rows={4} />
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button size="sm" onClick={handleSave}>{editingField ? "Save Changes" : "Create Field"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
